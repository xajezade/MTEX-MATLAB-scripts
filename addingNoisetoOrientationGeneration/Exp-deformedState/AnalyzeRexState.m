...
saveas(gcf,'Rex-ReferenceFrameAlignment.tiff','tiff');
close all
%% Plot raw data
fg=figure(1);
...
plot(ebsd,color)
saveas(fg,'Rex-raw-data.tiff','tiff');
close(fg);
...
plot(ebsd111,color)
saveas(fg,'Rex-111-fibre.tiff','tiff');
close(fg);
...
plot(ebsd001,color)
saveas(fg,'Rex-001-fibre.tiff','tiff');
close(fg);
...
plot(scater,color)
saveas(fg,'Rex-scatter.tiff','tiff');
close(fg);
% Plot pole figures with different colors
fg=figure(5);
plotIPDF(ebsd.orientations,yvector,'antipodal','all')
hold on
plotIPDF(scater.orientations,yvector,'antipodal','all','DisplayName','other orientations')
legend()
saveas(fg,'Rex-inverse polefigures.tiff','tiff');
close(fg);